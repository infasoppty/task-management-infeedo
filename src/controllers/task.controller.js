...
const {
  customResponse,
  getAllTasksCustomResponse,
  getLatestTaskCustomResponse,
  ...
  try {
    ...
    if (page || pageSize) {
      ...
      if (!page || !pageSize) {
        const response = customResponse(
          false,
          'Both "page" and "pageSize" query parameters are required'
        );
      ...
      const response = getAllTasksCustomResponse(
        true,
...
exports.getLatestTask = async (req, res) => {
  try {
    ...
    const response = getLatestTaskCustomResponse(
      true,
      "Latest task update retrieved successfully",
      latestTask
    );
    res.json(response);
  } catch (error) {
